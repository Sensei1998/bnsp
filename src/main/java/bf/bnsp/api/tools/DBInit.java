...
public class DBInit implements CommandLineRunner {
    ...
    private void initCaserne(){
        ...
        for(int i = 1; i < 101; i++){
            tmpAgent = new Agent("MT09062023" + String.format("%03d", i), "Admin" + String.format("%03d", i), "", encryption.encode("password"), "+22600001" + String.format("%03d", i), "admin"+(i)+"@bnsp.bf", this.caserneRepository.findById(i+1).get(), this.gradeRepository.findById(1).get());
            tmpAgent.setDefaultFonction(this.ruleRepository.findById(2).get());
            agents.add(tmpAgent);
        }
